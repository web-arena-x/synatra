...
def save_model(
    ...
    if not model_name:
        ...
        export_quantization_bit in GPTQ_BITS
        and not export_quantization_dataset
    ):
        error = ALERTS["err_no_dataset"][lang]
    elif export_quantization_bit not in GPTQ_BITS and not checkpoint_path:
        error = ALERTS["err_no_adapter"][lang]
    elif export_quantization_bit in GPTQ_BITS and isinstance(
...
def create_export_tab(engine: "Engine") -> Dict[str, "Component"]:
    ...
    checkpoint_path: gr.Dropdown = engine.manager.get_elem_by_id(
        "top.checkpoint_path"
    )
