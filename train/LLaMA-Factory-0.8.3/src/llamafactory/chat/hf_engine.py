...
class HuggingfaceEngine(BaseEngine):
    def __init__(
        ...
        self.semaphore = asyncio.Semaphore(
            int(os.environ.get("MAX_CONCURRENT", "1"))
        )
    ...
    def _process_args(
        ...
        if (
            ...
            messages[0]["content"] = (
                template.image_token + messages[0]["content"]
            )
        ...
        if (
            ...
            generating_args["temperature"] = (
                generating_args["temperature"] or 1.0
            )
    ...
    async def get_scores(
        ...
        **input_kwargs,
        ...
        async with self.semaphore:
            with concurrent.futures.ThreadPoolExecutor() as pool:
                return await loop.run_in_executor(
                    pool, self._get_scores, *input_args
                )
