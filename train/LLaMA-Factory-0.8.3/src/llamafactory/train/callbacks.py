...
def fix_valuehead_checkpoint(
    ...
    for name, param in state_dict.items():
        if name.startswith("v_head."):
            ...
            decoder_state_dict[
                name.replace("pretrained_model.", "", 1)
            ] = param
...
class SaveProcessorCallback(TrainerCallback):
    ...
    def on_train_end(
        ...
        **kwargs
        ...
        """
        if args.should_save:
            getattr(self.processor, "image_processor").save_pretrained(
                args.output_dir
            )
    ...
    def on_train_begin(
        ...
        **kwargs
        ...
        if args.should_save:
            ...
            logger.info(
                "Initial PiSSA adapter will be saved at: {}.".format(
                    pissa_init_dir
                )
            )
    ...
    def on_train_end(
        ...
        **kwargs
        ...
        if args.should_save:
            ...
            pissa_convert_dir = os.path.join(
                args.output_dir, "pissa_converted"
            )
            logger.info(
            ...
            if isinstance(model, PeftModel):
                ...
                model.load_adapter(
                    pissa_backup_dir, "default", is_trainable=True
                )
...
class LogCallback(TrainerCallback):
    def __init__(self) -> None:
        ...
        self.do_train = False
        """ Web UI """
        self.webui_mode = os.environ.get(
            "LLAMABOARD_ENABLED", "0"
        ).lower() in ["true", "1"]
        if self.webui_mode:
            signal.signal(signal.SIGABRT, self._set_abort)
            self.logger_handler = LoggerHandler(
                os.environ.get("LLAMABOARD_WORKDIR")
            )
    ...
    def on_log(
        ...
        **kwargs
        ...
        logs = dict(
            ...
            throughput="{:.2f}".format(
                state.num_input_tokens_seen / (time.time() - self.start_time)
            ),
