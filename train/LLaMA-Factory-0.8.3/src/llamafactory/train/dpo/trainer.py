...
class CustomDPOTrainer(DPOTrainer):
        ...
        if not self.finetuning_args.use_ref_model:
            if self.loss_type == "orpo":
                ...
                raise NotImplementedError(
                    "Unknown loss type: {}.".format(self.loss_type)
                )
    ...
    def concatenated_forward(
        ...
        all_logps, valid_length = get_batch_logps(
            logits=all_logits, labels=batch["labels"]
        )
    ...
    def get_batch_loss_metrics(
        ...
        if self.loss_type == "orpo":
            metrics["{}sft_loss".format(prefix)] = (
                sft_loss.detach().mean().cpu()
            )
            metrics["{}odds_ratio_loss".format(prefix)] = (
                ((losses - sft_loss) / self.beta).detach().mean().cpu()
            )
