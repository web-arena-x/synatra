...
def _process_request(
    ...
    List[Dict[str, str]], Optional[str], Optional[str], Optional["NDArray"]
]:
    logger.info(
        "==== request ====\n{}".format(
    ...
    for i, message in enumerate(request.messages):
        ...
        elif i % 2 == 1 and message.role not in [
            Role.ASSISTANT,
            Role.FUNCTION,
        ]:
            raise HTTPException(
    ...
    if isinstance(tool_list, list) and len(tool_list):
        try:
            tools = json.dumps(
                [dictify(tool.function) for tool in tool_list],
                ensure_ascii=False,
            )
...
async def create_chat_completion_response(
    ...
    for i, response in enumerate(responses):
        if tools:
            result = chat_model.engine.template.extract_tool(
...
async def create_score_evaluation_response(
    ...
    scores = await chat_model.aget_scores(
        request.messages, max_length=request.max_length
    )
