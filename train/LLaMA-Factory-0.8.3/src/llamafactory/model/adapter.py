...
from .model_utils.misc import (
    find_all_linear_modules,
    find_expanded_modules,
)
from .model_utils.quantization import QuantizationMethod
from .model_utils.unsloth import (
...
def _setup_freeze_tuning(
    ...
    if finetuning_args.use_llama_pro:
        ...
        trainable_layer_ids = range(
            min(-finetuning_args.freeze_trainable_layers, num_layers)
        )
        ...
        if module_name != "all" and module_name not in hidden_modules:
            raise ValueError(
                "Module {} is not found, please choose from {}".format(
                    module_name, ", ".join(hidden_modules)
                )
            )
    ...
    if (
        ...
        if finetuning_args.use_llama_pro:
            target_modules = find_expanded_modules(
                model, target_modules, finetuning_args.freeze_trainable_layers
            )

        if model_args.visual_inputs and finetuning_args.freeze_vision_tower:
            target_modules = "^(?!.*vision_tower).*(?:{}).*".format(
                "|".join(target_modules)
            )
        ...
        if model_args.use_unsloth:
            ...
            if finetuning_args.pissa_init:
                if finetuning_args.pissa_iter == -1:
                    ...
                    logger.info(
                        "Using PiSSA initialization with FSVD steps {}.".format(
                            finetuning_args.pissa_iter
                        )
                    )
                    peft_kwargs["init_lora_weights"] = "pissa_niter_{}".format(
                        finetuning_args.pissa_iter
                    )
...
def init_adapter(
    ...
    if finetuning_args.finetuning_type == "full":
        ...
        raise NotImplementedError(
            "Unknown finetuning type: {}.".format(
